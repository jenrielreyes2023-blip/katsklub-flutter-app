...
/// - Single persistent WebView that NEVER unmounts, preserving audio seamlessly!
/// - Smoothly transitions between Full-Screen Player and Bottom Mini-Player Bar.
/// - The mini-player retains a visible 72x48 video thumbnail so YouTube never pauses playback!
...
class _YouTubeMusicOverlayState extends State<YouTubeMusicOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: AnimatedPositioned(
        ...
        left: isMin ? 12.w : 0,
        right: isMin ? 12.w : 0,
        bottom: isMin ? (bottomPadding + 58.h) : 0,
        top: isMin ? (size.height - bottomPadding - 58.h - 64.h) : 0,
        child: Material(
          color: Colors.transparent,
          elevation: isMin ? 16 : 0,
          borderRadius: BorderRadius.circular(isMin ? 16.r : 0),
          child: Container(
            decoration: BoxDecoration(
              color: isMin ? const Color(0xFF181924) : const Color(0xFF0D0E15),
              borderRadius: BorderRadius.circular(isMin ? 16.r : 0),
              border: isMin
                  ? Border.all(color: const Color(0x33FF2A6D), width: 1.2)
                  : null,
              boxShadow: isMin
                  ? [
                      BoxShadow(
  ...
  Widget _buildMiniBar(dynamic track) {
    return Container(
      ...
      child: Row(
        children: [
          // Persistent visible mini video container (72x46)
          // Keeping this visible prevents YouTube from auto-pausing on minimize!
          ClipRRect(
